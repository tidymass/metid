...
#'
#' This function lists all the conflicts between packages in the metid
...
metid_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", metid_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "metid_conflicts")
}

metid_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "metid_conflicts()"
  )
...
#' @export
print.metid_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(metid_conflict_message(x))
}
